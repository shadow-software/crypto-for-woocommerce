...
final class BtcVerifier {
	...
	public function verify(): VerificationResult {
		...
		if ( '' === $pay_address || ! BtcAddress::is_valid( $pay_address ) ) {
			return VerificationResult::failed( __( 'This payment is missing its Bitcoin address.', 'shadowledger-crypto-for-woocommerce' ) );
		}
		...
		try {
			...
			return VerificationResult::pending( __( 'Checking the Bitcoin network… we could not reach an explorer just now and will retry shortly.', 'shadowledger-crypto-for-woocommerce' ) );
		}
	}
	...
	private function verify_by_sender_scan( string $sender, string $pay_address, string $min_sats ): VerificationResult {
		...
		return VerificationResult::pending( __( 'Looking for your payment on the Bitcoin network. If you have paid, this usually clears within a few minutes.', 'shadowledger-crypto-for-woocommerce' ) );
	}
	...
	private function evaluate_tx( array $tx, string $txid, string $pay_address, string $sender, string $min_sats ): VerificationResult {
		...
		if ( '0' === $paid_sats ) {
			return VerificationResult::failed( __( 'That transaction did not pay the store address. Please check and try again.', 'shadowledger-crypto-for-woocommerce' ) );
		}
		...
		if ( Money::compare( $paid_sats, $min_sats ) < 0 ) {
			return VerificationResult::failed(
				sprintf(
					...
					__( 'That payment was for %1$s BTC but %2$s BTC is required. If you underpaid, please send the difference in a new transaction.', 'shadowledger-crypto-for-woocommerce' ),
		...
		if ( ! $confirmed ) {
			return VerificationResult::pending( __( 'Payment found — waiting for it to be included in a block.', 'shadowledger-crypto-for-woocommerce' ) );
		}

		$block_height = isset( $status['block_height'] ) ? (int) $status['block_height'] : 0;

		if ( $block_height <= 0 ) {
			return VerificationResult::pending( __( 'Payment found — confirming. Please wait a moment.', 'shadowledger-crypto-for-woocommerce' ) );
		}
		...
		if ( $start_height > 0 && $block_height < $start_height ) {
			return VerificationResult::failed( __( 'That transaction was confirmed before this order was placed, so it cannot pay for it. Please make a new payment.', 'shadowledger-crypto-for-woocommerce' ) );
		}
