...
final class Gateway extends \WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->title       = $this->get_option( 'title', __( 'Pay with crypto (ETH, USDC, USDT, BTC)', 'shadowledger-crypto-for-woocommerce' ) );
		$this->description = $this->get_option( 'description', __( 'Pay in crypto from your own wallet. After you place the order we show the exact amount and address to pay.', 'shadowledger-crypto-for-woocommerce' ) );
	...
	public function init_form_fields(): void {
		$fields = array(
			'enabled'           => array(
				'title'   => __( 'Enable/Disable', 'shadowledger-crypto-for-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable crypto payments', 'shadowledger-crypto-for-woocommerce' ),
				'default' => 'no',
			),
			'title'             => array(
				'title'       => __( 'Title', 'shadowledger-crypto-for-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'Payment method title shown to the customer at checkout.', 'shadowledger-crypto-for-woocommerce' ),
				'default'     => __( 'Pay with crypto (ETH, USDC, USDT, BTC)', 'shadowledger-crypto-for-woocommerce' ),
				'desc_tip'    => true,
			),
			'description'       => array(
				'title'       => __( 'Description', 'shadowledger-crypto-for-woocommerce' ),
				'type'        => 'textarea',
				'description' => __( 'Payment method description shown to the customer at checkout.', 'shadowledger-crypto-for-woocommerce' ),
				'default'     => __( 'Pay in crypto from your own wallet. After you place the order we show the exact amount and address to pay.', 'shadowledger-crypto-for-woocommerce' ),
			),
			'wallet'            => array(
				'title'       => __( 'Ethereum & tokens — receiving wallet', 'shadowledger-crypto-for-woocommerce' ),
				'type'        => 'title',
				'description' => __( 'The 0x… address every EVM customer pays (ETH, USDC and USDT all arrive at this one address on whichever network the buyer chooses). Use an address you control. Double-check it — funds are sent by customers directly and cannot be recovered by this plugin.', 'shadowledger-crypto-for-woocommerce' ),
			),
			'receiving_address' => array(
				'title'             => __( 'EVM receiving address (0x…)', 'shadowledger-crypto-for-woocommerce' ),
				'type'              => 'text',
				'description'       => __( 'Your 0x… address. If you paste a checksummed (mixed-case) address it is verified; a typo will be rejected on save. Leave blank to disable ETH/USDC/USDT.', 'shadowledger-crypto-for-woocommerce' ),
				'default'           => '',
			...
			'networks_title'    => array(
				'title'       => __( 'Networks & assets', 'shadowledger-crypto-for-woocommerce' ),
				'type'        => 'title',
				'description' => __( 'Choose which networks customers may pay on, and which assets (ETH / USDC / USDT) to accept on each. All use free public nodes by default; you can point any network at your own RPC endpoint below.', 'shadowledger-crypto-for-woocommerce' ),
			),
		);
		...
		foreach ( Networks::all() as $slug => $network ) {
			$fields[ 'network_' . $slug . '_enabled' ] = array(
				...
				'label'   => sprintf( __( 'Accept payments on %s', 'shadowledger-crypto-for-woocommerce' ), $network['name'] ),
			...
			$fields[ 'asset_eth:' . $slug . '_enabled' ] = array(
				/* translators: %s: network name. */
				'title'   => sprintf( __( '%s — assets', 'shadowledger-crypto-for-woocommerce' ), $network['name'] ),
				'type'    => 'checkbox',
				'label'   => __( 'ETH (native)', 'shadowledger-crypto-for-woocommerce' ),
				'default' => 'yes',
			);
			foreach ( Assets::all() as $asset ) {
				...
				$fields[ 'asset_' . $asset['id'] . '_enabled' ] = array(
					...
					'label'   => sprintf( __( '%s (token)', 'shadowledger-crypto-for-woocommerce' ), $asset['symbol'] ),
					'default' => 'yes',
				);
			}
			...
			$fields[ 'network_' . $slug . '_confirmations' ] = array(
				/* translators: %s: network name. */
				'title'             => sprintf( __( '%s — confirmations', 'shadowledger-crypto-for-woocommerce' ), $network['name'] ),
				'type'              => 'number',
				'description'       => __( 'Block confirmations required before the order is completed. Higher is safer but slower.', 'shadowledger-crypto-for-woocommerce' ),
			...
			$fields[ 'network_' . $slug . '_rpc' ]           = array(
				/* translators: %s: network name. */
				'title'       => sprintf( __( '%s — custom RPC URL', 'shadowledger-crypto-for-woocommerce' ), $network['name'] ),
				'type'        => 'text',
				/* translators: %s: the default public RPC URL for the network. */
				'description' => sprintf( __( 'Optional. Leave blank to use the free public node (%s).', 'shadowledger-crypto-for-woocommerce' ), $network['default_rpc'] ),
				'default'     => '',
		...
		$fields['btc_title']         = array(
			'title'       => __( 'Bitcoin', 'shadowledger-crypto-for-woocommerce' ),
			'type'        => 'title',
			'description' => __( 'Accept native BTC to your own Bitcoin address, confirmed via free public block explorers. Leave the address blank to disable Bitcoin.', 'shadowledger-crypto-for-woocommerce' ),
		);
		$fields['btc_enabled']       = array(
			'title'   => __( 'Bitcoin', 'shadowledger-crypto-for-woocommerce' ),
			'type'    => 'checkbox',
			'label'   => __( 'Accept native Bitcoin (BTC)', 'shadowledger-crypto-for-woocommerce' ),
			'default' => 'no',
		);
		$fields['btc_address']       = array(
			'title'             => __( 'BTC receiving address', 'shadowledger-crypto-for-woocommerce' ),
			'type'              => 'text',
			'description'       => __( 'Your Bitcoin mainnet address (bc1…, 1… or 3…). The checksum is verified on save; a typo will be rejected.', 'shadowledger-crypto-for-woocommerce' ),
			'default'           => '',
			'placeholder'       => 'bc1…',
			'custom_attributes' => array(
				'autocomplete'   => 'off',
		...
		$fields['btc_confirmations'] = array(
			'title'             => __( 'Bitcoin — confirmations', 'shadowledger-crypto-for-woocommerce' ),
			'type'              => 'number',
			'description'       => __( 'Confirmations required before a Bitcoin order is completed. 1–2 is common for small amounts; more is safer.', 'shadowledger-crypto-for-woocommerce' ),
			'default'           => '2',
			'desc_tip'          => true,
			'custom_attributes' => array(
				'min'  => '1',
		...
		$fields['btc_explorer']      = array(
			'title'       => __( 'Bitcoin — custom explorer URL', 'shadowledger-crypto-for-woocommerce' ),
			'type'        => 'text',
			'description' => __( 'Optional. An Esplora-compatible API base URL (e.g. your own mempool instance). Leave blank to use the free mempool.space and blockstream.info explorers.', 'shadowledger-crypto-for-woocommerce' ),
			'default'     => '',
		...
		$fields['pricing_title']  = array(
			'title'       => __( 'Pricing & tolerance', 'shadowledger-crypto-for-woocommerce' ),
			'type'        => 'title',
			'description' => __( 'The order total is converted from your store currency to the chosen crypto at the live market rate when the buyer picks how to pay, and that amount is locked for them.', 'shadowledger-crypto-for-woocommerce' ),
		);
		$fields['tolerance_bps']  = array(
			'title'             => __( 'Underpayment tolerance (%)', 'shadowledger-crypto-for-woocommerce' ),
			'type'              => 'number',
			'description'       => __( 'Accept a payment this many percent below the quoted amount, to absorb rate drift and rounding. 0 requires the exact amount or more.', 'shadowledger-crypto-for-woocommerce' ),
			'default'           => '1',
			'desc_tip'          => true,
			'custom_attributes' => array(
				'min'  => '0',
		...
		$fields['window_minutes'] = array(
			'title'             => __( 'Payment window (minutes)', 'shadowledger-crypto-for-woocommerce' ),
			'type'              => 'number',
			'description'       => __( 'How long to keep polling the chain for a submitted payment before the order is marked failed.', 'shadowledger-crypto-for-woocommerce' ),
			'default'           => '60',
			'desc_tip'          => true,
			'custom_attributes' => array(
				'min'  => '10',
	...
	public function validate_receiving_address_field( $key, $value ) {
		...
		if ( ! Address::is_valid_format( $value ) ) {
			\WC_Admin_Settings::add_error( __( 'The EVM receiving address is not a valid 0x… address. It was not saved.', 'shadowledger-crypto-for-woocommerce' ) );

			return (string) $this->get_option( $key, '' );
		}

		if ( ! Address::is_valid_checksum( $value ) ) {
			\WC_Admin_Settings::add_error( __( 'The EVM receiving address has an invalid checksum (likely a typo). It was not saved.', 'shadowledger-crypto-for-woocommerce' ) );
	...
	public function validate_btc_address_field( $key, $value ) {
		...
		if ( ! BtcAddress::is_valid( $value ) ) {
			\WC_Admin_Settings::add_error( __( 'The Bitcoin receiving address is not a valid mainnet address (bc1…, 1… or 3…). It was not saved.', 'shadowledger-crypto-for-woocommerce' ) );
			...
			<div class="shadow-eth-admin__head">
				...
				<div class="shadow-eth-admin__titles">
					...
					<p class="shadow-eth-admin__tagline">
						<?php esc_html_e( 'Self-custodial crypto checkout — ETH, USDC, USDT & BTC — free and open, by Shadow Software LLC.', 'shadowledger-crypto-for-woocommerce' ); ?>
					</p>
				</div>
			</div>
			<div class="shadow-eth-admin__pills">
				<span class="shadow-eth-admin__pill"><?php esc_html_e( 'Your wallet, your keys', 'shadowledger-crypto-for-woocommerce' ); ?></span>
				<span class="shadow-eth-admin__pill"><?php esc_html_e( 'On-chain verified', 'shadowledger-crypto-for-woocommerce' ); ?></span>
				<span class="shadow-eth-admin__pill shadow-eth-admin__pill--neutral"><?php esc_html_e( 'Free public nodes', 'shadowledger-crypto-for-woocommerce' ); ?></span>
				<span class="shadow-eth-admin__pill shadow-eth-admin__pill--neutral"><?php esc_html_e( 'No fees · No middleman', 'shadowledger-crypto-for-woocommerce' ); ?></span>
			</div>
			<p class="shadow-eth-admin__notice">
				<?php esc_html_e( 'Payments are sent by customers directly to your wallet and settle on a public blockchain. On-chain payments are irreversible and cannot be refunded or recovered by this plugin — treat a confirmed order like cash, and always confirm your receiving address is correct.', 'shadowledger-crypto-for-woocommerce' ); ?>
			</p>
			<div class="shadow-eth-admin__meta">
				<span class="shadow-eth-admin__version">
					<?php
					/* translators: %s: the installed plugin version. */
					echo esc_html( sprintf( __( 'Version %s', 'shadowledger-crypto-for-woocommerce' ), SHADOW_ETH_VERSION ) );
					?>
				</span>
				<span class="sh-sep" aria-hidden="true">·</span>
				<a href="<?php echo esc_url( self::SUPPORT_URL ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Documentation', 'shadowledger-crypto-for-woocommerce' ); ?></a>
				<a href="<?php echo esc_url( self::SUPPORT_URL ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Support', 'shadowledger-crypto-for-woocommerce' ); ?></a>
				<span class="sh-sep" aria-hidden="true">·</span>
				<span class="shadow-eth-admin__by">
					<?php
					printf(
						/* translators: %s: the plugin author, linked. */
						esc_html__( 'by %s', 'shadowledger-crypto-for-woocommerce' ),
	...
	public function process_payment( $order_id ): array {
		...
		if ( ! $order instanceof \WC_Order ) {
			return $this->fail( __( 'Order not found.', 'shadowledger-crypto-for-woocommerce' ) );
		}

		if ( empty( $this->get_enabled_assets() ) ) {
			return $this->fail( __( 'This store has not finished setting up crypto payments. Please choose another method.', 'shadowledger-crypto-for-woocommerce' ) );
		}
		...
		if ( $order->has_status( array( 'pending', 'failed' ) ) ) {
			$order->update_status(
				'pending',
				__( 'Awaiting crypto payment. Buyer will choose an asset and pay on the order page.', 'shadowledger-crypto-for-woocommerce' )
			);
		}
	...
	public function lock_asset_amount( \WC_Order $order, string $asset_id ): string {
		...
		if ( null === $asset || ! $this->is_asset_enabled( $asset_id ) ) {
			return __( 'Please choose a valid payment option.', 'shadowledger-crypto-for-woocommerce' );
		}

		$address = $this->get_asset_receiving_address( $asset );

		if ( '' === $address ) {
			return __( 'This payment option is not available right now. Please choose another.', 'shadowledger-crypto-for-woocommerce' );
		}
		...
		if ( null === $units || '0' === $units ) {
			$order->add_order_note( __( 'Accept Crypto: could not fetch the live exchange rate to price this order.', 'shadowledger-crypto-for-woocommerce' ) );

			return __( 'We could not fetch the live exchange rate. Please try again in a moment or choose another option.', 'shadowledger-crypto-for-woocommerce' );
		}
		...
		if ( Money::compare( $units, $quoted ) < 0 ) {
			...
			return __( 'We could not price this order correctly. Please try again or choose another option.', 'shadowledger-crypto-for-woocommerce' );
		}
