...
final class PaymentChecker {
	...
	public function run_check( $order_id ): void {
		...
		if ( $this->window_elapsed( $order, $gateway ) ) {
			$this->fail_order( $order, __( 'We did not detect a matching payment within the payment window.', 'shadow-software-crypto-for-woocommerce' ) );
	...
	private function complete_order( \WC_Order $order, VerificationResult $result ): void {
		...
		if ( ! TxRegistry::claim( $network, $result->tx_hash(), $order->get_id() ) ) {
			...
			$this->fail_order( $order, __( 'This transaction has already been used to pay another order.', 'shadow-software-crypto-for-woocommerce' ) );
		...
		$net_name    = null !== $network_obj ? $network_obj['name'] : ( 'bitcoin' === $network ? __( 'Bitcoin', 'shadow-software-crypto-for-woocommerce' ) : $network );

		$note = sprintf(
			/* translators: 1: asset symbol, 2: network name, 3: confirmations, 4: transaction hash. */
			__( 'Accept Crypto: %1$s payment confirmed on %2$s with %3$d confirmations. Tx: %4$s', 'shadow-software-crypto-for-woocommerce' ),
			$symbol,
			$net_name,
	...
	private function fail_order( \WC_Order $order, string $message ): void {
		...
		$order->update_status(
			...
			sprintf(
				...
				__( 'Accept Crypto: %s', 'shadow-software-crypto-for-woocommerce' ),
				$message
			)
		);
		$order->save();
