...
final class RpcClient {
	...
	public function assert_chain(): void {
		...
		if ( ! is_string( $result ) || 1 !== preg_match( '/^0x[0-9a-fA-F]+$/', $result ) ) {
			throw new RpcException( esc_html__( 'Could not verify the network of the RPC endpoint.', 'shadow-software-crypto-for-woocommerce' ) );
		}
			...
			throw new RpcException(
				esc_html(
					sprintf(
						...
						__( 'RPC endpoint is on the wrong network (expected chain %1$d, got %2$d).', 'shadow-software-crypto-for-woocommerce' ),
						$expected,
						$actual
					)
				)
	...
	public function block_number(): int {
		...
		if ( ! is_string( $result ) || 1 !== preg_match( '/^0x[0-9a-fA-F]+$/', $result ) ) {
			throw new RpcException( esc_html__( 'Invalid block number from RPC endpoint.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( strlen( $hex ) > 15 ) {
			throw new RpcException( esc_html__( 'Out-of-range block number from RPC endpoint.', 'shadow-software-crypto-for-woocommerce' ) );
		}
