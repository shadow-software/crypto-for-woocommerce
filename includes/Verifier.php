...
final class Verifier {
	...
	public function verify(): VerificationResult {
		...
		if ( ! Networks::is_supported( $this->network ) || '' === $pay_address ) {
			return VerificationResult::failed( __( 'This payment is missing its network configuration.', 'shadowpay-crypto-for-woocommerce' ) );
		}
		...
		try {
			...
			return VerificationResult::pending( __( 'Checking the network… we could not reach a node just now and will retry shortly.', 'shadowpay-crypto-for-woocommerce' ) );
		}

		return VerificationResult::pending( __( 'Waiting for your payment details.', 'shadowpay-crypto-for-woocommerce' ) );
	}
	...
	private function verify_by_tx_hash( string $tx_hash, string $pay_address, string $sender, string $min_wei ): VerificationResult {
		...
		if ( $this->is_erc20() ) {
			...
			if ( ! Address::equals( $to, $this->token_contract() ) ) {
				return VerificationResult::failed( __( 'That transaction is not a payment of the requested token. Please check and try again.', 'shadowpay-crypto-for-woocommerce' ) );
			}
		...
		if ( ! Address::equals( $to, $pay_address ) ) {
			return VerificationResult::failed( __( 'That transaction did not pay the store wallet. Please check the transaction and try again.', 'shadowpay-crypto-for-woocommerce' ) );
		}
	...
	private function underpaid_result( string $paid_units, string $required_units ): VerificationResult {
		return VerificationResult::failed(
			sprintf(
				...
				__( 'That payment was for %1$s %3$s but %2$s %3$s is required. If you underpaid, please send the difference in a new transaction.', 'shadowpay-crypto-for-woocommerce' ),
	...
	private function finalise_with_receipt( string $tx_hash, string $pay_address = '', string $sender = '', string $min_wei = '' ): VerificationResult {
		...
		if ( null === $receipt ) {
			return VerificationResult::pending( __( 'Your payment was found and is waiting to be mined into a block.', 'shadowpay-crypto-for-woocommerce' ) );
		}
		...
		if ( 0 === $status ) {
			return VerificationResult::failed( __( 'That transaction failed on-chain (it was reverted), so no payment was received.', 'shadowpay-crypto-for-woocommerce' ) );
		}

		if ( 1 !== $status ) {
			return VerificationResult::pending( __( 'Your payment is confirming. Please wait a moment.', 'shadowpay-crypto-for-woocommerce' ) );
		}
	...
	private function receipt_has_token_payment( array $receipt, string $pay_address, string $sender, string $min_units ): ?VerificationResult {
		...
		if ( ! $found ) {
			return VerificationResult::failed( __( 'That transaction did not include a token payment to the store from your wallet. Please check and try again.', 'shadowpay-crypto-for-woocommerce' ) );
		}
	...
	private function verify_by_sender_scan( string $sender, string $pay_address, string $min_wei ): VerificationResult {
		...
		if ( $head < $cursor ) {
			return VerificationResult::pending( __( 'Looking for your payment on the network. Please wait a moment.', 'shadowpay-crypto-for-woocommerce' ) );
		}
		...
		return VerificationResult::pending( __( 'Looking for your payment on the network. If you have paid, this usually clears within a few minutes.', 'shadowpay-crypto-for-woocommerce' ) );
	}
	...
	private function scan_token_logs( string $sender, string $pay_address, string $min_units, int $from, int $to, int $cursor ): VerificationResult {
		...
		if ( '' === $from_topic || '' === $to_topic ) {
			return VerificationResult::failed( __( 'This order is missing valid payment addresses.', 'shadowpay-crypto-for-woocommerce' ) );
		}
