...
final class Verifier {
	...
	public function verify(): VerificationResult {
		...
		if ( ! Networks::is_supported( $this->network ) || '' === $pay_address ) {
			return VerificationResult::failed( __( 'This payment is missing its network configuration.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( '0' === $min_wei ) {
			...
			return VerificationResult::failed( __( 'This order is missing its payment amount. Please contact us to complete it.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		try {
			...
			return VerificationResult::pending( __( 'Checking the network… we could not reach a node just now and will retry shortly.', 'shadow-software-crypto-for-woocommerce' ) );
		}

		return VerificationResult::pending( __( 'Waiting for your payment details.', 'shadow-software-crypto-for-woocommerce' ) );
	}
	...
	private function verify_by_tx_hash( string $tx_hash, string $pay_address, string $sender, string $min_wei ): VerificationResult {
		...
		if ( '' === $sender ) {
			return VerificationResult::failed( __( 'We need the wallet address you paid from to confirm this payment.', 'shadow-software-crypto-for-woocommerce' ) );
		}

		$tx = $this->rpc->get_transaction( $tx_hash );

		if ( null === $tx ) {
			return VerificationResult::pending( __( 'Your transaction has not appeared on the network yet. This can take a moment after you send it.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( ! Address::equals( $from, $sender ) ) {
			return VerificationResult::failed( __( 'That transaction was not sent from the wallet you entered. Please check and try again.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( $this->is_erc20() ) {
			...
			if ( ! Address::equals( $to, $this->token_contract() ) ) {
				return VerificationResult::failed( __( 'That transaction is not a payment of the requested token. Please check and try again.', 'shadow-software-crypto-for-woocommerce' ) );
			}
		...
		if ( ! Address::equals( $to, $pay_address ) ) {
			return VerificationResult::failed( __( 'That transaction did not pay the store wallet. Please check the transaction and try again.', 'shadow-software-crypto-for-woocommerce' ) );
		}
	...
	private function underpaid_result( string $paid_units, string $required_units ): VerificationResult {
		return VerificationResult::failed(
			sprintf(
				...
				__( 'That payment was for %1$s %3$s but %2$s %3$s is required. If you underpaid, please send the difference in a new transaction.', 'shadow-software-crypto-for-woocommerce' ),
	...
	private function finalise_with_receipt( string $tx_hash, string $pay_address = '', string $sender = '', string $min_wei = '' ): VerificationResult {
		...
		if ( ! TxRegistry::is_available_for( $this->network, $tx_hash, $this->order->get_id() ) ) {
			return VerificationResult::failed( __( 'That transaction has already been used to pay another order. Please make a new payment for this order.', 'shadow-software-crypto-for-woocommerce' ) );
		}

		$receipt = $this->rpc->get_transaction_receipt( $tx_hash );

		if ( null === $receipt ) {
			return VerificationResult::pending( __( 'Your payment was found and is waiting to be mined into a block.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( 0 === $status ) {
			return VerificationResult::failed( __( 'That transaction failed on-chain (it was reverted), so no payment was received.', 'shadow-software-crypto-for-woocommerce' ) );
		}

		if ( 1 !== $status ) {
			return VerificationResult::pending( __( 'Your payment is confirming. Please wait a moment.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		if ( $confs < $required ) {
			return VerificationResult::pending(
				sprintf(
					...
					__( 'Payment found — waiting for network confirmations (%1$d of %2$d).', 'shadow-software-crypto-for-woocommerce' ),
					$confs,
					$required
				),
				$tx_hash,
	...
	private function receipt_has_token_payment( array $receipt, string $pay_address, string $sender, string $min_units ): ?VerificationResult {
		...
		if ( ! $found ) {
			return VerificationResult::failed( __( 'That transaction did not include a token payment to the store from your wallet. Please check and try again.', 'shadow-software-crypto-for-woocommerce' ) );
		}
	...
	private function verify_by_sender_scan( string $sender, string $pay_address, string $min_wei ): VerificationResult {
		...
		if ( $head < $cursor ) {
			return VerificationResult::pending( __( 'Looking for your payment on the network. Please wait a moment.', 'shadow-software-crypto-for-woocommerce' ) );
		}
		...
		return VerificationResult::pending( __( 'Looking for your payment on the network. If you have paid, this usually clears within a few minutes.', 'shadow-software-crypto-for-woocommerce' ) );
	}
	...
	private function scan_token_logs( string $sender, string $pay_address, string $min_units, int $from, int $to, int $cursor ): VerificationResult {
		...
		if ( '' === $from_topic || '' === $to_topic ) {
			return VerificationResult::failed( __( 'This order is missing valid payment addresses.', 'shadow-software-crypto-for-woocommerce' ) );
		}
